    ...
    <>
      ...
      <main className="mx-auto max-w-5xl px-6 py-16 sm:px-10 sm:py-20">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Badge variant={listing.status === "Published" ? "accent" : "muted"}>
            {listing.status}
          </Badge>
        </div>

        <div className="mt-10 flex flex-wrap items-end justify-between gap-6 border-b border-[var(--color-border)] pb-10">
          <div>
            ...
            <p className="mt-3 small-caps text-xs text-[var(--color-muted-foreground)]">
        ...
        {error && (
          <p
            role="alert"
            className="mt-6 border border-[var(--color-destructive)]/30 bg-[var(--color-destructive)]/5 px-4 py-3 text-sm text-[var(--color-destructive)]"
          >
            {error}
          </p>
        )}
          ...
          <div className="grid gap-8 md:grid-cols-[200px_1fr] md:gap-12">
            ...
            <div>
              {listing.photos.length === 0 ? (
                <div className="mb-6 border border-dashed border-[var(--color-border)] bg-[var(--color-muted)]/40 px-6 py-16 text-center">
                ...
                <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-3">
                  {listing.photos.map((p, i) => (
                    <figure
                      key={p.id}
                      className="group relative aspect-[4/3] overflow-hidden bg-[var(--color-muted)]"
                    >
                      ...
                      />
                      {i === 0 && (
                        <span className="pointer-events-none absolute left-2 top-2 inline-flex items-center rounded-[4px] bg-[var(--color-accent)] px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.14em] text-[var(--color-accent-foreground)]">
                          Cover
                        </span>
                      )}
                      <button
                        ...
                        className="absolute right-2 top-2 inline-flex h-8 items-center justify-center rounded-[4px] bg-[var(--color-card)]/95 px-3 text-xs font-medium text-[var(--color-destructive)] opacity-0 transition-opacity duration-200 ease-out group-hover:opacity-100 focus-visible:opacity-100"
                        aria-label="Remove photo"
                      >
                        Remove
                      </button>
                      {p.caption && (
                        <figcaption className="absolute inset-x-0 bottom-0 truncate bg-gradient-to-t from-[var(--color-foreground)]/70 to-transparent px-3 py-2 text-xs text-[var(--color-sand)]">
                          {p.caption}
                        </figcaption>
                      )}
                    </figure>
              ...
              <div className="grid gap-4 sm:grid-cols-[1fr_auto] sm:items-end">
                ...
                <label
                  className={`inline-flex h-11 cursor-pointer items-center justify-center gap-2 rounded-[4px] border border-[var(--color-foreground)] bg-transparent px-5 text-sm font-medium text-[var(--color-foreground)] transition-colors duration-200 ease-out hover:bg-[var(--color-foreground)] hover:text-[var(--color-sand)] ${uploading ? "pointer-events-none opacity-60" : ""}`}
                >
